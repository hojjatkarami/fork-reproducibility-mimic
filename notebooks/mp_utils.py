...
def generate_times(df, T=None, T_to_death=None, seed=None, censor=False):
    ...
    idx = (~df['deathtime_hours'].isnull()) & (
        df['deathtime_hours'] < df['dischtime_hours'])
    ...
    if censor:
        idx = (~df['censortime_hours'].isnull()) & (
            df['censortime_hours'] < df['endtime'])
    ...
    if T_to_death is not None:
        ...
        idxInICU = (df['deathtime_hours'] -
                    df['dischtime_hours']) <= T_to_death
        # for these patients, set the time to be T_to_death hours
        df.loc[idxInICU, 'windowtime'] = df.loc[idxInICU,
                                                'deathtime_hours'] - T_to_death
...
def print_cm(y, yhat):
    ...
    print('y=0\t{:6g}\t{:6g}\tSpec={:2.2f}').format(
        cm[0, 0], cm[0, 1], 100.0*TN/(TN+FP))  # Spec
    print('y=1\t{:6g}\t{:6g}\tSens={:2.2f}').format(
        cm[1, 0], cm[1, 1], 100.0*TP/(TP+FN))  # Sens
    # add sensitivity/specificity as the bottom line
    print('   \t{:2.2f}\t{:2.2f}\t Acc={:2.2f}').format(
        100.0*TN / (TN+FN), 100.0*TP / (TP+FP), 100.0*(TP+TN)/N)
    print('   \tNPV\tPPV')
...
def vars_of_interest():
    ...
    var_last = var_first
    var_last.extend(
        ['gcsmotor', 'gcsverbal', 'gcseyes', 'endotrachflag', 'gcs'])
...
def get_design_matrix(df, time_dict, W=8, W_extra=24):
    ...
    df_time = pd.DataFrame(data=tmp, index=None, columns=[
                           'icustay_id', 'hr', 'early_flag'])
    df_time.sort_values(['icustay_id', 'hr'], inplace=True)

    # merge df_time with df to filter down to a subset of rows
    df = df.merge(df_time, left_on=['icustay_id', 'hr'], right_on=[
                  'icustay_id', 'hr'], how='inner')
    ...
    df_first_early.columns = [
        x + '_first_early' for x in df_first_early.columns]
    ...
    df_data = pd.concat([df_first, df_first_early, df_last,
                        df_last_early, df_min, df_max, df_sum], axis=1)
...
def debug_for_iid(df, time_dict, iid, T=8, W_extra=24):
    ...
    display(HTML(df.loc[df['icustay_id'] == iid,
            :].to_html().replace('NaN', '')))

    # display the times selected for this icustay_id
    print('\n\n TIMES FOR THIS ICUSTAY_ID \n\n')
    display(HTML(df_time.loc[df_time['icustay_id']
            == iid].to_html().replace('NaN', '')))

    # merge df_time with df to filter down to a subset of rows
    df = df.loc[df['icustay_id'] == iid, :].merge(
        df_time, left_on=['icustay_id', 'hr'], right_on=['icustay_id', 'hr'], how='inner')
    df = df.loc[df['early_flag'] == 0, :]
    display(HTML(df.to_html().replace('NaN', '')))

    print('\n\nFIRST\n\n')
    display(HTML(df_tmp.groupby('icustay_id')[
            var_first].first().to_html().replace('NaN', '')))
    print('\n\nLAST\n\n')
    display(HTML(df_tmp.groupby('icustay_id')[
            var_first].last().to_html().replace('NaN', '')))
...
def plot_vitals(df, iid, df_death=None, df_censor=None):
    ...
    plt.plot(df.loc[idx, 'hr'], df.loc[idx, 'heartrate'],
             'ro-', label='Heart rate')
    plt.plot(df.loc[idx, 'hr'], df.loc[idx, 'sysbp'],
             'b^', label='Systolic BP', alpha=0.5)
    plt.plot(df.loc[idx, 'hr'], df.loc[idx, 'diasbp'],
             'bv', label='Diastolic BP', alpha=0.5)
    plt.plot(df.loc[idx, 'hr'], df.loc[idx, 'meanbp'],
             'bd', label='Mean BP', alpha=0.8)
    plt.plot(df.loc[idx, 'hr'], df.loc[idx, 'resprate'],
             'go', label='Respiratory rate', alpha=0.5)

    if df_death is not None:
        # add in discharge/death time
        idx = df_death['icustay_id'] == iid
        plt.plot(np.ones([2,])*df_death.loc[idx, 'dischtime_hours'].values,
                 [0, 200], 'k--', linewidth=2, label='Time of discharge')
        if df_death.loc[idx, 'deathtime_hours'] is not np.nan:
            plt.plot(np.ones([2,])*df_death.loc[idx, 'deathtime_hours'].values,
                     [0, 200], 'k-', linewidth=2, label='Time of death')
            plt.plot(np.ones([2,])*df_death.loc[idx, 'deathtime_hours'].values -
                     24, [0, 200], 'k:', linewidth=2, label='24 hr before death')
...
def plot_model_results(results, pretty_labels=None):
    if pretty_labels is None:
        pretty_labels = {'xgb': 'GB', 'rf': 'RF',
                         'logreg': 'LR', 'lasso': 'LASSO'}
...
def load_design_matrix(co, df_additional_data=None, data_ext='', path=None, diedWithin=None):
    ...
    df_offset = df_offset.loc[:, ['icustay_id',
                                  'intime', 'outtime', 'starttime', 'deathtime']]
    ...
    if df_additional_data is not None:
        df = df.merge(df_additional_data, how='left',
...
def get_predictions(df, df_static,  mdl, iid):
    ...
    for t in tm:
        ...
        X = X.merge(df_static.set_index('icustay_id')[
                    var_static], how='left', left_index=True, right_index=True)
...
def get_data_at_time(df, df_static, iid, hour=0):
    ...
    if len(idx) == 0:
        ...
        if len(idx) == 0:
            ...
            idx = idx[-1]
        print(
            'Hour not found! Using closest previous value: {}.'.format(tm[idx]))
    else:
        idx = idx[0]
